...
ITEM_NAMES = ("쌀", "감자", "배추", "무", "양파", "깐마늘", "토마토", "사과", "배", "고등어")
...
class FixtureCodexRunner:
    ...
    def run(
        ...
        self.calls += 1
        manifest_path = schema_path.parent / "manifest.json"
        ...
        fieldnames = tuple(
            line.split("`")[1]
            for line in (
                Path(".agents/skills/coupang-ui-collector/references/csv-schema.md")
                ...
                writer.writerow(
                    {
                        ...
                        "query": target["query"],
                        "product_id": f"p-{target['item_code']}-{target['kind_code']}",
                        "title": title,
                        ...
                        "raw_accessible_name": f"{title} 10000원",
                    }
                )
...
def test_pipeline_collects_only_missing_coupang_rows_and_persists_csv(
    tmp_path: Path,
) -> None:
    ...
    repository.save_daily(
        ...
        [
            PlatformPriceSummary(
                "coupang", "111", "10", Decimal(33000), 5, 5, (), ()
            )
        ],
        TODAY,
        "seed-run",
    ...
    result = service.collect(
        catalog_entries(), TODAY, "run-2", include_combined=False
    )
