...
def collect_naver_agent(
    container: ApplicationContainer,
    observed_date: date,
    *,
    target: tuple[str, str] | None = None,
) -> OnlineCollectionResult:
    source = container.naver_agent_source
    repository = container.online_repository
    if source is None or repository is None or not container.settings.naver_agent_enabled:
        raise RuntimeError("Naver agent collection is unavailable")
    ...
    run = CollectionRun.start("naver_agent", observed_date, observed_date)
    ...
    try:
        ...
        errors = tuple(
            CollectionError("naver_agent", "SourceFailure", message)
        ...
        container.run_repository.save(
            run.finish(
                ...
                error_count=1,
                errors=(
                    CollectionError("naver_agent", type(error).__name__, str(error)),
                ),
            )
        )
        raise
...
def main(argv: Sequence[str] | None = None) -> int:
    settings = Settings.from_env()
    parser = argparse.ArgumentParser(description="Collect Naver through Codex agent")
    parser.add_argument("--date", type=date.fromisoformat)
    parser.add_argument("--item", type=parse_target)
    arguments = parser.parse_args(argv)
    observed_date = arguments.date or local_today(settings.timezone)
    result = run_naver_agent(observed_date, target=arguments.item)
